...
Every built-in rule, its defaults, and every option it accepts. How to write
the config itself is covered in [configuration.md](./configuration.md); the
reasoning behind each rule lives in its linked guideline, also served by
`vibator explain <rule>`. Rules with a default severity of `off` need the
options marked required before they can act.
...
| Option                | Type     | Default              | Purpose                                                      |
|-----------------------|----------|----------------------|--------------------------------------------------------------|
| `patterns`            | object[] | —                    | The patterns to ban, each carrying its own diagnostic text   |
| `patterns[].pattern`  | string   | —                    | JavaScript regular expression source, matched per line       |
| `patterns[].flags`    | string   | `""`                 | Regular expression flags, such as i                          |
| `patterns[].message`  | string   | —                    | What is wrong when the pattern matches                       |
| `patterns[].expected` | string   | —                    | The standard, positively stated                              |
| `patterns[].fix`      | string   | —                    | The concrete next action                                     |
...
| Option    | Type                                        | Default                  | Purpose                                                                                                 |
|-----------|---------------------------------------------|--------------------------|---------------------------------------------------------------------------------------------------------|
| `root`    | string                                      | —                        | Directory holding the locale catalogs                                                                   |
...
| `locales` | string[]                                    | —                        | The locales to check; discovered from the layout when omitted                                           |
...
| Option                   | Type     | Default  | Purpose                                                  |
|--------------------------|----------|----------|----------------------------------------------------------|
| `generators`             | object[] | —        | Generators to run, each with the paths it owns           |
| `generators[].name`      | string   | —        | Human-readable name, used in messages                    |
| `generators[].command`   | string   | —        | Shell command that regenerates the output                |
| `generators[].cwd`       | string   | `"."`    | Working directory, relative to the project root          |
| `generators[].paths`     | string[] | —        | Paths the generator writes, relative to the project root |
