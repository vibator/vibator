...
The built-in rules are an opinionated starting set. Every project has standards
only it can state — a naming convention for one directory, a boundary a linter
cannot see, a file that must never import another. Those belong in a rule of
your own.

**First check you need one.** A standard a regular expression over lines can
express — a forbidden import, a banned call, a TODO without a ticket — belongs
in the built-in `banned-patterns` rule's options, in pure JSON, with no code to
maintain. Write a rule for what patterns cannot see: types, scopes, cross-file
questions.
...
A rule is a plain object. Nothing is registered globally, nothing is inherited,
and there is no base class.
...
| `defaultInclude`  | Globs selecting the files the rule judges.                            |
...
`defineRule` is identity at runtime; it exists so `options` is typed inside your
check from the schema you declared.
...
engine iterates and reports progress for you, so counters and bars are accurate
without you emitting anything.

```ts
checkFile({file, bytes, options, context}): Diagnostic[]
```

**`scope: "project"`** — implement `check`, called once with every selected
file. Use it for questions no single file can answer: whether two locales agree,
whether generated output still matches its source, whether exactly one module
declares something. May be async.
...
Prefer `file` scope. It is simpler, it parallelises conceptually, and it gets
progress for free. Reach for `project` only when the question genuinely spans
files.
...
The three message fields are the point of this tool. Keep them distinct.
...
- **`fix`** — the concrete next action. A person or an agent should be able to
  act on this line alone, without opening the guideline.

An agent consuming `--reporter json` reads these as three fields. Collapsing
them into one prose sentence is the failure mode this design exists to avoid.
...
TypeScript program is expensive, and several rules asking for the same tsconfig
share one type-check between them rather than paying for one each.

---

## Working with TypeScript

`typescript` is an optional peer dependency, so load it on demand rather than
importing it at module scope. That way a project using only your text rules
...
Only reach for `context.program(tsconfig)` when you need to *resolve* something
— what a symbol refers to, which overload a call selects, what type an
expression has. It costs seconds rather than milliseconds.

Two traps worth knowing, both learned from the built-in `no-deprecated-apis`:
...
- **A symbol merges the documentation of every overload.** Asking whether
  ...
  tag-name overload nobody called. Resolve the signature at the call site
...
If your rule can be wrong, give it a marked exception rather than making people
disable it wholesale. The convention is a comment on the line above, with a
mandatory reason:
...
Accept the marker as an option so a project adopting your rule can keep whatever
marker its source already uses:
...
The bare marker must not match — require the trailing reason. An unexplained
exemption is the drift the rule exists to stop.

For line-based rules, `hasLineIgnoreAbove(lines, lineNumber, markers)` from the
`vibator` package implements the convention — `//`, `#` and `<!--` comment
leaders, reason required — so plugins do not each grow their own dialect.
...
Write it for the person who just hit the rule and disagrees. A good guideline
answers four questions, in this order:

1. **What is the rule**, in one sentence.
2. **Why it exists** — the failure it prevents. Be concrete about what breaks
   and where. "It works in the browser and fails in the native shell" persuades;
   "consistency" does not.
3. **What is expected** — ideally with the corrected code.
4. **When an exception is legitimate**, and how to mark it.

A rule whose guideline cannot answer question 2 is usually a preference, and
preferences belong in a formatter.
...
`plugins` accepts repo-relative paths or package names. TypeScript files work
directly on Node 22.18+, which strips types natively.
...
Rule ids must be unique across built-ins and plugins. A collision is an error,
not a silent override — otherwise config would mean different things depending
on load order.

If your rules live in their own directory inside a repo that is not itself an ES
module, add a `package.json` there with `{"type": "module"}`.

---

## Testing

Rules are pure functions. Test them by calling them directly — no fixtures
directory, no runner integration.
...
Test the boundary, not just the hit: the case that should pass is where false
positives get caught. For every rule, cover at least
...
Before writing one, check it clears these:

- **Deterministic.** Same input, same finding. No clock, no network, no
  randomness — a gate that flickers gets disabled.
- **Actionable.** You can state the fix in one line. If you cannot, the problem
  is a design discussion, not a lint rule.
- **Low false positives.** A rule that cries wolf gets switched off, and takes
  its true positives with it. When in doubt, ship it as `warn` first.
- **Not already covered.** Formatters own formatting. Type checkers own types.
  Write the rule your other tools structurally cannot see.

The best candidates are the standards you find yourself repeating in code
review. Those are, by definition, the ones people do not internalise from
documentation alone.
