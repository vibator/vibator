...
import { packageManifest, packageRoot } from "../core/package-root.ts";
import type { AnyRule } from "../core/rule.ts";

/**
 * The `$schema` value that will actually resolve for this install.
 *
 * @remarks the local `node_modules` schema path is checked rather than
...
function schemaReference(root: string): string {
  ...
  const local = `./node_modules/${name}/schema.json`;
  if (existsSync(resolve(root, "node_modules", name, "schema.json"))) {
    return local;
  }
  if (existsSync(join(packageRoot, "schema.json"))) {
    return `https://unpkg.com/${name}@${version}/schema.json`;
  }
  return local;
}
