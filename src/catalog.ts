 ...
 */
function defaultOf(node: SchemaNode): string {
 ...
 *
 * @param properties - The property nodes.
...
function rowsOf(
  ...
  return Object.entries(properties).flatMap(([key, node]) => {
    ...
    const row: OptionRow = {
      name,
      type: typeOf(node),
      defaultValue: defaultOf(node),
      description: node.description ?? "",
    };
    const nested = node.items?.properties ?? node.properties;
    const nestedPrefix = node.items?.properties ? `${name}[]` : name;
    return nested ? [row, ...rowsOf(nested, nestedPrefix)] : [row];
  });
}
...
function optionsTableOf(rule: AnyRule): string {
  ...
  const rows = rowsOf(schema.properties ?? {});
